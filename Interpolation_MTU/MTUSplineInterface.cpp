...
void MTUSplineInterface::initialisation(OpenSim::GeometryPath const& gp,
                                        std::vector<OpenSim::Coordinate const*> coords,
                                        SimTK::State& st,
                                        std::vector<int>& nPoints)
{
    splineData_ = new MTUSplineDataRead(gp,coords,st,nPoints);
    splineData_->readTaskCoefficients();
    taskMTU_ = splineData_->getTaskVect();

    muscleNames_.push_back(gp.getName());
    for (unsigned i=0; i<coords.size(); i++){
        dofNames_.push_back(coords[i]->getName());
        musclesNamesOnDof_.push_back({gp.getName()});
    }
