...
function OutputData = IntegrateOpenSimPlant(osimModel, controlsFuncHandle,...
    ...
    % passed as arguments
    plantHandle = @(t,Y) OpenSimPlantFunction(t, Y, controlsFuncHandle, osimModel, osimState);
...
function status = OpenSimVisualizeFunction(t, Y, flag, opensimModel, ...
    ...
    if(isempty(flag))
        ...
        vectroYhandle = opensimState.updY();
        for i = 1:1:stateSize,
            vectroYhandle.set(i, Y(i));
        end
